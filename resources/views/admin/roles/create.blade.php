...
@section('page-header')
    <div class="row mb-2 mb-xl-3">
        <div class="col-auto d-none d-sm-block">
            <h3><strong>{{__('admin/pages/roles.title')}}</strong> / {{__('admin/pages/roles.add')}}</h3>
        </div>
    </div>
@endsection
@section('content')
    ...
    <form action="{{ route('roles.store') }}" method="post" class="needs-validation" novalidate id="myForm">
        @csrf
        <div class="row">
            <input class="form-control form-control-lg mb-3" type="text" placeholder="{{ __('admin/pages/roles.role.name') }}" name="name" required autocomplete="off">
        </div>
        <div class="row">
            <div class="list-group-item">
                <input type="checkbox" id="checkAll" name="checkAll">
                <label for="checkAll">{{ __('admin/pages/roles.select.all') }}</label>
            </div>
        </div>
        <br>
                    ...
                    <div class="col-12 col-md-6 col-lg-3">
                        <div class="card">
                            <div class="card-header" style="background: #21beb5">
                                <h5 class="card-title mb-0" style="color: white">{{ __('admin/pages/roles.' . $model) }}</h5>
                            </div>
                            <ul class="list-group list-group-flush">
                                @foreach($methods as $method)
                                    ...
                                    <li class="list-group-item">
                                        ...
                                        <label for="{{ $model . $method }}"> {{ __('admin/pages/roles.' . $method) }}</label>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
        ...
        <button type="submit" class="btn btn-primary">{{__('admin/pages/roles.add')}}</button>
    </form>
