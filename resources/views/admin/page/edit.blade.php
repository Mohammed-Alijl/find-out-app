...
@section('title',__('admin/pages/pages.edit.title'))
@section('css')
...
@section('page-header')
    <div class="row mb-2 mb-xl-3">
        <div class="col-auto d-none d-sm-block">
            <h3><strong>{{__('admin/pages/pages.title')}}</strong> / {{__('admin/pages/pages.edit.page')}}
            </h3>
        </div>
    </div>
@endsection
@section('content')
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title">{{__('admin/pages/pages.edit.title')}}</h5>
                <h6 class="card-subtitle text-muted">{{__('admin/pages/pages.edit.description')}}</h6>
            </div>
            <div class="card-body">
                <form action="{{route('pages.update',$page->id)}}" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-6">
                            ...
                            <input type="text" class="form-control" id="name_ar" name="name_ar" maxlength="255" value="{{$page->getTranslation('name','ar')}}"
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <input type="text" class="form-control" id="name_en" name="name_en" maxlength="255" value="{{$page->getTranslation('name','en')}}"
                    ...
                    <div class="file-loading">
                        <input id="input-pd" name="image" type="file">
                    </div>
                    <br>
                    <label class="form-label"
                           for="content">{{__('admin/pages/pages.content')}} <span style="color: red">*</span></label>
                    <textarea name="content" id="content" cols="30" rows="70" class="form-control">{{$page->content}}</textarea>
                    <br>
                    <button type="submit"
                            class="btn btn-primary">{{__('admin/pages/pages.edit')}}</button>
                </form>
            </div>
        </div>
    </div>
    <script>
        $("#input-pd").fileinput({
            ...
            initialPreview: [
                    "{{ asset('storage/img/' . $page->image_path) }}",
            ],
            initialPreviewConfig: [
                {
                    caption: '{{ $page->image_path }}',
                },
            ],
